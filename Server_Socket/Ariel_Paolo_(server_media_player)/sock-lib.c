...
/* Asignamos las opciones al socket */	
        int optval;
...
int Open_conection ( struct sockaddr_in * my_addr)
{
	...
	if ((sockaux = socket(AF_INET, SOCK_STREAM, 0)) == -1) //ints( IPv4 TCP)
	{
...
/* Asignamos las opciones al socket */	
	int optval;
	setsockopt(sockaux, SOL_SOCKET, SO_REUSEADDR, &optval, sizeof optval);
